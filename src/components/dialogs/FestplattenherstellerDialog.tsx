...
interface FestplattenherstellerDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Festplattenhersteller['fields']) => Promise<void>;
  defaultValues?: Festplattenhersteller['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function FestplattenherstellerDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: FestplattenherstellerDialogProps) {
  const [fields, setFields] = useState<Partial<Festplattenhersteller['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'festplattenhersteller');
      await onSubmit(clean as Festplattenhersteller['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "hersteller_name": string | null, // Herstellername\n  "hersteller_kuerzel": string | null, // Kürzel / Markenzeichen\n  "herkunftsland": string | null, // Herkunftsland\n  "gruendungsjahr": number | null, // Gründungsjahr\n  "webseite": string | null, // Webseite\n  "email_kontakt": string | null, // Kontakt-E-Mail\n  "anmerkungen": string | null, // Anmerkungen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Festplattenhersteller['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Festplattenhersteller bearbeiten' : 'Festplattenhersteller hinzufügen';
      ...
      <DialogContent className="max-w-lg max-h-[92vh] flex flex-col overflow-hidden p-0 gap-0">
        ...
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col min-h-0">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="hersteller_name">Herstellername</Label>
            <Input
              id="hersteller_name"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="hersteller_kuerzel">Kürzel / Markenzeichen</Label>
            <Input
              id="hersteller_kuerzel"
              value={fields.hersteller_kuerzel ?? ''}
              onChange={e => setFields(f => ({ ...f, hersteller_kuerzel: e.target.value }))}
            />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="herkunftsland">Herkunftsland</Label>
            <Input
              id="herkunftsland"
              value={fields.herkunftsland ?? ''}
              onChange={e => setFields(f => ({ ...f, herkunftsland: e.target.value }))}
            />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="email_kontakt">Kontakt-E-Mail</Label>
            <Input
              id="email_kontakt"
              type="email"
              value={fields.email_kontakt ?? ''}
              onChange={e => setFields(f => ({ ...f, email_kontakt: e.target.value }))}
            />
          </div>
          <div className="space-y-1.5">
